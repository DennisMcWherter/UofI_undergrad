...
public class MainFeudalView {
	...
	public MainFeudalView(int x, int y) {
		...
		this.aiMove = new JButton(MenuItems.FEUDAL_BUTTON_AI);
		this.aiMove.addActionListener(this.controller);
		this.aiMove.setEnabled(false);
	...
	private void setupMenu(JFrame window) {
		...
		JMenu net  = new JMenu("Networking");
		...
		JMenuItem join   = new JMenuItem(MenuItems.FEUDAL_CONNECT);
		JMenuItem host   = new JMenuItem(MenuItems.FEUDAL_HOST);
		...
		join.addActionListener(this.controller);
		host.addActionListener(this.controller);
	 ...
	 * @param networked Is this a networked game?
	 */
	public void setupNextMove(List<Integer> indices, boolean networked) {
		this.start.setEnabled(false);
		this.finish.setEnabled(true);
		if(!networked)
			this.aiMove.setEnabled(true);
		else
			this.aiMove.setEnabled(false);
	...
	public void removeListeners() {
		...
		for(int i = 0 ; i < FeudalBoard.BOARD_SIZE*FeudalBoard.BOARD_SIZE ; ++i) {
			JPanel square = (JPanel)this.main.getComponent(i);
			for(MouseListener m : square.getMouseListeners())
				square.removeMouseListener(m);
		}
	}
	...
	/**
	 * Draw the map of the current board
	 */
	public void drawMap() {
		...
		this.board.add(this.aiMove, BorderLayout.AFTER_LINE_ENDS);
		this.board.add(this.main);
