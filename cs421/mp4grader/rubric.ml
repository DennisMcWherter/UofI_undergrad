...
let extra_rubric = [
  ...
  "import_list"^" "^"[1;2;2;2;3;4;5;2;1]", mptest 1 (ss_pair1 Solution.import_list Student.import_list ([1;2;2;2;3;4;5;2;1]));
  "import_list"^" "^"[]", mptest 1 (ss_pair1 Solution.import_list Student.import_list ([]));
  "import_list"^" "^"[1;(-2);(-1);7]", mptest 1 (ss_pair1 Solution.import_list Student.import_list ([1;(-2);(-1);7]));
  "elem_sol"^" "^"5"^" "^"[1;2;(-1);3;4]", mptest 1 (ss_pair2 elem_sol elem_stu (5) ([1;2;(-1);3;4]));
  "elem_sol"^" "^"7"^" "^"[(-2);1;2;3;4]", mptest 1 (ss_pair2 elem_sol elem_stu (7) ([(-2);1;2;3;4]));
  "elem_sol"^" "^"1"^" "^"[]", mptest 1 (ss_pair2 elem_sol elem_stu (1) ([]));
  "num_of_consts"^" "^"IfExp (BinExp (Eq,VarExp \"l\", ConExp Nil), ConExp (Int 0), ConExp(Bool(false)))", mptest 1 (ss_pair1 Solution.num_of_consts Student.num_of_consts (IfExp (BinExp (Eq,VarExp "l", ConExp Nil), ConExp (Int 0), ConExp(Bool(false)))));
  "cps_sol"^" "^"((LetExp(\"a\", FunExp(\"b\", MonExp(Head, VarExp \"b\")), BinExp(Add, VarExp \"a\", ConExp (Int 5)))) )", mptest 1 (ss_pair1 cps_sol cps_stu (((LetExp("a", FunExp("b", MonExp(Head, VarExp "b")), BinExp(Add, VarExp "a", ConExp (Int 5)))) )));
  "cps_sol"^" "^"(IfExp(MonExp(Tail, VarExp \"x\"), FunExp(\"x\", BinExp(Eq, VarExp \"x\", ConExp(Int 5))), AppExp(FunExp(\"p\", MonExp(Print, VarExp \"p\")), ConExp(Bool false))))", mptest 1 (ss_pair1 cps_sol cps_stu ((IfExp(MonExp(Tail, VarExp "x"), FunExp("x", BinExp(Eq, VarExp "x", ConExp(Int 5))), AppExp(FunExp("p", MonExp(Print, VarExp "p")), ConExp(Bool false))))));
  "cps_sol"^" "^"(RecExp(\"f'\", \"x\", FunExp(\"p\", BinExp(Add, VarExp \"p\", VarExp \"x\")), ConExp(Int 12)))", mptest 1 (ss_pair1 cps_sol cps_stu ((RecExp("f'", "x", FunExp("p", BinExp(Add, VarExp "p", VarExp "x")), ConExp(Int 12)))))
]
