...
#define TIMEOUT (3)
#define FORMAT_PRINT(a, b, c, d, e, f, g) if(isServer) \
      cout<< a << setw(16) << \
      b << setw(16) << \
      c << setw(16) << \
      d << setw(16) << \
...
TCP::TCP(const char* host, const char* port, bool server)
  : sockfd(-1), host(host), port(port), phase("SLOW START"), isServer(server),
  cwnd(MSS), ssthresh(MAX_WND), canSend(cwnd), RTT(1.f), RTO(1000000),
  lastAck(0), lastSent(0), dupackCount(0), retransmitting(false), inprocofrtrans(false),
  closing(false)
{
  TCP::instance = this;
  mp3_init();
//  signal(SIGINT, TCP::timeout_handler); // Use the timeout callback to handle signals
  if(isServer) {
    FORMAT_PRINT("Timestamp", "Segment", "Send", "Recv", "Phase", "cwnd", "ssthresh");
    cout<< setfill('-') << setw(105) << "-" << setfill(' ') << endl << endl;
  } else {
    cout<< "Trying to receive file from server..." << endl;
  }
}
...
int TCP::send(int sock, void* data, size_t len)
{
  ...
  while(keepSending) {
    ...
    tv.tv_sec  = 0;
    tv.tv_usec = 350000; // TODO - update RTO
...
int TCP::processAck(Packet& ack)
{
  ...
  // if timed out, then can send calculated differently?
  if(ack.start > lastAck) { // Not a dupack
    ...
    canSend = cwnd - (lastSent - lastAck) - 1;
  ...
  phase = (retransmitting) ? "FAST RETRANS." : (slowstart) ? "SLOW START" : "CONG. AVOID";

  FORMAT_PRINT(getRelativeTime(), ack.start, "-", "ACK", phase, cwnd, ssthresh);
...
int TCP::consumeSendBuffer(int val, int sock, ClientState* state)
{
  ...
  while(!q.empty()) {
    ...
    if(segend < val) {
      ...
      canSend += q.front().size;
      canSend = (canSend > cwnd) ? cwnd : canSend;
      q.pop();
    } else {
      cum += q.front().size;
...
int TCP::sendPacket(int sock, ClientState* state, Packet& packet)
{
  ...
  if(ret >= 0 && !(packet.metadata & PACKET_CONTROL)) {
    if(!(packet.metadata & PACKET_RETRANSMIT)) {
      ...
      lastSent = packet.start + packet.size;
    }

    // Queue this but set that it's retransmitted if sent again
    packet.metadata |= PACKET_RETRANSMIT;
    sendBuffer[sock].push(packet);
    const char* myPhase = (retransmitting) ? "FAST RETRANS." : (cwnd <= ssthresh) ? "SLOW START" : "CONG. AVOID";
    FORMAT_PRINT(getRelativeTime(), packet.start+1, "Send", "-", myPhase, cwnd, ssthresh);
...
int TCP::recvPacket(int sock, ClientState* state, Packet& packet, struct sockaddr& theirs, socklen_t& tlen)
{
  ...
  if(getNextRQueue(sock, packet)) {
    ret = packet.size;
    state->seqno += ret;
//    cout<< "Updating seqno to: " << state->seqno << " ; packet.start: " << packet.start << endl;
  }
  ...
  while(ret == -5) {
    ...
    if(ret == sizeof(packet)) { // Received a whole packet
      ...
      if(!(packet.metadata & PACKET_CONTROL)) {
        ...
        ack.start = ackNo; // Field shows what we are ack'ing
...
bool TCP::getNextRQueue(int sock, Packet& packet)
{
  ...
  ClientState* state = getState(sock);
  stack<Packet> pollution;

  if(queue.empty() || state == NULL)
    return false;

  while(!queue.empty()) {
    pollution.push(queue.top());
    queue.pop();
  }
  ...
  while(!pollution.empty() && pollution.top().start < state->seqno)
    pollution.pop();

#if 0
  //cout<< "Looking" << endl;
  cout<< "seqno: " << state->seqno << " ; " << queue.top().start << endl;
  ...
  bool ret = false;
  if(!pollution.empty() && pollution.top().start == state->seqno) {
    packet = Packet(pollution.top());
    pollution.pop();
    while(!pollution.empty() && pollution.top().start == packet.start)
      pollution.pop();
    ret = true;
  }

  while(!pollution.empty()) {
    queue.push(pollution.top());
    pollution.pop();
  }
...
string TCP::getRelativeTime() const
{
  struct timeval tv;
...
unsigned TCP::cumulativeAck()
{
  ClientState* state = getState(sockfd);

  assert(state);

  p_pq& q = recvBuffer[sockfd];
  stack<Packet> st;
  unsigned ackNo = state->seqno;

  if(q.empty())
    return state->seqno + 1;
