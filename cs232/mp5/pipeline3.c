...
main(int argc, char **argv) {
  const unsigned PREFETCH_SIZE = 10;
  ...
  for (int i = 0 ; i < num_elements ; i ++) {
   ...
   __builtin_prefetch(list[i+PREFETCH_SIZE], 1, 0);
  }
  ...
  for (int step = 0 ; step < NUM_STEPS ; step ++) {
	 ...
	 for (int i = 0 ; i < num_elements ; i ++) {
      ...
      transform(list2[num_elements2], vp);
      update(list2[num_elements2], e_update);
		  num_elements2 ++;
		}
    __builtin_prefetch(list[i+PREFETCH_SIZE], 0, 0);
   }
  }
}
