...
m_loop:	
	...
	blt	$a0, 80, next		# already on my side
...
check_clue:
  ...
  li	$t7, 10	# Turtlize me cap'n ( d3ad in my tracks :'[  )		
	sw	$t7, 0xffff0010($zero)		# set velocity
  ...
  add $t6, $a0, $0
	add $a0, $t5, $0
	jal count_nodes
	li	$t7, 10		# B4ck t0 r4bb1t sp33d	(He's alive!!!!!)	
	sw	$t7, 0xffff0010($zero)		# set velocity
...
still_pushing:
	...
	beq	$t4, 1, next	
	lw	$t2, 0xffff0020($zero)	# get my X position
	bgt	$t2, 10, still_pushing	# keep going until I pass block
...
bonk_interrupt:
  ...
  # Chekizzle bizonkizzle ( from Rohan da Shohan cuddy gee ----- wizeeeeeeeeek <3 )
  ble $t6, 0, dizont_bizonk
  bge $t6, 295, dizont_bizonk
  ble $t8, 0, dizont_bizonk
  bge $t8, 295, dizont_bizonk
